...
from quant_data.stage3 import compare_clean_stage3_rebuilds, run_clean_stage3_rebuild
from tests.runtime_data_guard import assert_project_data_unchanged, snapshot_project_data


PROJECT_ROOT = Path(__file__).resolve().parents[1]
GOLDEN_PATH = PROJECT_ROOT / "tests" / "fixtures" / "stage3_golden.json"
STORE_FILENAMES = {
    "market.sqlite",
    "macro.sqlite",
    "company.sqlite",
...
class Stage3IntegrationTests(unittest.TestCase):
    ...
    def test_two_clean_rebuilds_match_the_approved_stage3_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(evidence["contract"], "quant_data.stage3_rebuild_evidence")
            ...
            actual = {
                ...
                "stage2_evidence_sha256": evidence["stage2_evidence_sha256"],
                "stage3_fixture_evidence_sha256": evidence[
                    "stage3_fixture_evidence"
                ]["sha256"],
            ...
            for rebuild in ("first", "second"):
                for cohort in ("stage2/source", "backup", "restored"):
                    self.assertEqual(
                        {
                            path.name
    ...
    def test_clean_rebuild_refuses_a_nonempty_explicit_work_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ConflictError):
                run_clean_stage3_rebuild(project_root=PROJECT_ROOT, work_root=root)
