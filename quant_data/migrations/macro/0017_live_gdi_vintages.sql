-- Extend the live-vintage model for BEA gross domestic income.
-- Existing migrations remain immutable. This successor adds the two GDI
-- series already present in the official GDP/GDI vintage workbook and widens
-- the existing BEA provenance lineage gate to cover them.
...
CREATE TABLE macro_live_vintage_captures_0017 (
...
INSERT INTO macro_live_vintage_captures_0017 (
...
ALTER TABLE macro_live_vintage_captures_0017 RENAME TO macro_live_vintage_captures;
...
CREATE TABLE macro_live_vintage_series_0017 (
    ...
    CHECK (
        ...
        OR
        (series_id = 'macro.gdi.real_qoq_saar_pct'
         AND provider = 'bea' AND provider_series_code = 'A261RL'
         AND frequency = 'quarterly' AND unit = 'percent'
         AND value_representation = 'rate' AND availability_basis = 'source_release')
        OR
        (series_id = 'macro.gdi.nominal_billions'
         AND provider = 'bea' AND provider_series_code = 'A261RC'
         AND frequency = 'quarterly' AND unit = 'billions_usd'
         AND value_representation = 'level' AND availability_basis = 'source_release')
        OR
...
INSERT INTO macro_live_vintage_series_0017 (
...
ALTER TABLE macro_live_vintage_series_0017 RENAME TO macro_live_vintage_series;
