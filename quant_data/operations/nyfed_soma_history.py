"""Manual-only, bounded NY Fed SOMA summary history operation."""
...
NYFED_SOMA_URL: Final = "https://markets.newyorkfed.org/api/soma/summary.json"
NYFED_SOMA_COLLECTOR_ID: Final = "nyfed.macro.soma_summary_history"
NYFED_SOMA_HANDLER: Final = "macro.nyfed_soma_summary_history"
...
class NyFedSomaWindow:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValidationError("NY Fed SOMA window is invalid")


@dataclass(frozen=True, slots=True)
class NyFedSomaHistoryReport:
    requested: int
    ...
    def mapping(self) -> dict[str, object]:
        return {
            ...
            "unchanged_releases": self.unchanged_releases,
            "written_components": self.written_components,
...
ParseNyFedSoma = Callable[..., object]
...
def _require_bound_target(
    *, project_root: Path, macro_store: Path, canonical: bool
    ...
    try:
        ...
        raise StoreUnavailableError("NY Fed SOMA target is unavailable") from exc
    if (
        root != project_root
        ...
        raise ValidationError("NY Fed SOMA target binding is invalid")
    if canonical:
        if project_root != PROJECT_ROOT or macro_store != MACRO_STORE:
            raise ValidationError("NY Fed SOMA canonical target binding is invalid")
        return
    try:
        project_root.relative_to(Path("/tmp").resolve(strict=True))
    except ValueError as exc:
        raise ValidationError("NY Fed SOMA fixtures must use a temporary root") from exc
...
def _publication_result(value: object) -> tuple[int, int, int, str, str]:
    published = getattr(value, "published_releases", None)
    unchanged = getattr(value, "unchanged_releases", None)
    written = getattr(value, "written_components", None)
    first = getattr(value, "first_as_of_date", None)
    last = getattr(value, "last_as_of_date", None)
    if (
        ...
        or unchanged < 0
        or published + unchanged < 1
        or isinstance(written, bool)
        or not isinstance(written, int)
        or written < 0
        or (published == 0 and written != 0)
        or not isinstance(first, str)
        or not isinstance(last, str)
    ):
        raise ConflictError("NY Fed SOMA publisher outcome is invalid")
    return published, unchanged, written, first, last


class NyFedSomaHistoryRunner:
    """Fetch and publish exactly one bounded NY Fed SOMA response."""
    ...
    def _request(self) -> FmpMacroCalendarTransportResponse:
    ...
    def run(
        ...
        *,
        start_date: date | str,
        end_date: date | str,
    ) -> NyFedSomaHistoryReport:
        window = NyFedSomaWindow(
        ...
        published, unchanged, written, first, last = _publication_result(
            publisher.publish(capture)
        )
        return NyFedSomaHistoryReport(
            requested=1,
...
def _domain_api() -> tuple[ParseNyFedSoma, PublisherFactory]:
    from ..macro.nyfed_soma_summary import (
        NyFedSomaPublisher,
    ...
    def publisher_factory() -> NyFedSomaPublisher:
        return NyFedSomaPublisher(
            project_root=PROJECT_ROOT,
            macro_store=MACRO_STORE,
            registry=registry,
            _canonical=True,
    ...
    return parse_nyfed_soma_summary, publisher_factory


def populate_nyfed_soma_live(
    *, start_date: date | str, end_date: date | str
) -> NyFedSomaHistoryReport:
    """Run one explicit canonical NY Fed SOMA summary history request."""

    _require_canonical_target()
    parser, publisher_factory = _domain_api()
    return NyFedSomaHistoryRunner(
        project_root=PROJECT_ROOT,
        macro_store=MACRO_STORE,
        parser=parser,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        report = populate_nyfed_soma_live(
    ...
    sys.stderr.write(
        dumps_strict(
            {
                "contract": "quant_data.nyfed_soma_history_error",
                "error": name,
                "exit_code": code,
                "version": _VERSION,
            }
...
__all__ = (
    "MACRO_STORE",
    "MAX_WINDOW_DAYS",
    "NYFED_SOMA_COLLECTOR_ID",
    "NYFED_SOMA_HANDLER",
    "NYFED_SOMA_URL",
    "NyFedSomaHistoryReport",
    "NyFedSomaHistoryRunner",
    "NyFedSomaWindow",
    "PROJECT_ROOT",
    "populate_nyfed_soma_live",
)
