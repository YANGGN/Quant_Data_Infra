"""Manual-only, bounded NY Fed repo-facility history operation."""
...
NYFED_REPO_FACILITIES_URL: Final = (
    "https://markets.newyorkfed.org/api/rp/results/search.json"
)
NYFED_REPO_FACILITIES_COLLECTOR_ID: Final = (
    "nyfed.macro.repo_facility_usage_history"
)
NYFED_REPO_FACILITIES_HANDLER: Final = (
...
class NyFedRepoFacilitiesWindow:
    start_date: date
    end_date: date
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValidationError("NY Fed repo-facility window is invalid")
...
@dataclass(frozen=True, slots=True)
class NyFedRepoFacilitiesHistoryReport:
    requested: int
    published: int
    unchanged: int
    written_series: int
...
ParseNyFedRepoFacilities = Callable[..., object]
...
def _require_bound_target(
    *, project_root: Path, macro_store: Path, canonical: bool
    ...
    try:
        ...
        raise StoreUnavailableError("NY Fed repo-facility target is unavailable") from exc
    if (
        root != project_root
        ...
        raise ValidationError("NY Fed repo-facility target binding is invalid")
    if canonical:
        if project_root != PROJECT_ROOT or macro_store != MACRO_STORE:
            raise ValidationError(
                "NY Fed repo-facility canonical target binding is invalid"
            )
        return
    try:
        ...
        raise ValidationError(
            "NY Fed repo-facility fixtures must use a temporary root"
        ) from exc
...
class NyFedRepoFacilitiesHistoryRunner:
    """Fetch and publish exactly one bounded NY Fed repo-results response."""
    ...
    def __init__(
        ...
        *,
        ...
        parser: ParseNyFedRepoFacilities,
    ...
    def _request(
        self, window: NyFedRepoFacilitiesWindow
    ) -> FmpMacroCalendarTransportResponse:
        response = self._transport.request(
            url=NYFED_REPO_FACILITIES_URL,
    ...
    def run(
        ...
        *,
        start_date: date | str,
        end_date: date | str,
    ) -> NyFedRepoFacilitiesHistoryReport:
        window = NyFedRepoFacilitiesWindow(
        ...
        return NyFedRepoFacilitiesHistoryReport(
            requested=1,
...
def _domain_api() -> tuple[ParseNyFedRepoFacilities, PublisherFactory]:
    ...
    def publisher_factory() -> NyFedRepoFacilitiesPublisher:
        return NyFedRepoFacilitiesPublisher(
            project_root=PROJECT_ROOT,
            macro_store=MACRO_STORE,
            registry=registry,
            _canonical=True,
    ...
    return parse_nyfed_repo_facilities, publisher_factory


def populate_nyfed_repo_facilities_live(
    *, start_date: date | str, end_date: date | str
) -> NyFedRepoFacilitiesHistoryReport:
    """Run one explicit canonical NY Fed repo-facility history request."""

    _require_canonical_target()
    parser, publisher_factory = _domain_api()
    return NyFedRepoFacilitiesHistoryRunner(
        project_root=PROJECT_ROOT,
        macro_store=MACRO_STORE,
        parser=parser,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        report = populate_nyfed_repo_facilities_live(
    ...
    sys.stderr.write(
        dumps_strict(
            {
                "contract": "quant_data.nyfed_repo_facilities_history_error",
                "error": name,
                "exit_code": code,
                "version": _VERSION,
            }
...
__all__ = (
    "MACRO_STORE",
    "MAX_WINDOW_DAYS",
    "NYFED_REPO_FACILITIES_COLLECTOR_ID",
    "NYFED_REPO_FACILITIES_HANDLER",
    "NYFED_REPO_FACILITIES_URL",
    "NyFedRepoFacilitiesHistoryReport",
    "NyFedRepoFacilitiesHistoryRunner",
    "NyFedRepoFacilitiesWindow",
    "PROJECT_ROOT",
    "populate_nyfed_repo_facilities_live",
)
